export function initializeAditorPoint(inputSection, defaultValue) {
  ...
  function activeOption() {
    // const options = inputSection.querySelector(".dropdownMenu");
    const allActiveOptions = dropdownMenu.querySelectorAll("[class$=active]");
  ...
  aditor.addEventListener("keydown", (e) => {
    if (e.key === "/") {
      ...
      if (range) {
        if (!subLineActive) {
          parentRect = current_Editable.getBoundingClientRect();
        } else {
          parentRect = current_subEditable.getBoundingClientRect();
        }

        // Position dropdown UNDER `current_Editable`
        dropdownMenu.style.top = `${window.scrollY + parentRect.bottom + 10}px`;
        // dropdownMenu.style.left = `${window.scrollX + parentRect.left}px`;
        // dropdownMenu.style.width = `${parentRect.width}px`;

        // Show dropdown
        currentFocus = currentLabelActive();
        preFocus = currentFocus;
...
export function initializeAditor(inputSection, defaultValue) {
  ...
  const dateBtn = inputSection.querySelector(".dateBtn");
  ...
  inputSection.addEventListener("keydown", (e) => {
    if (dropdownMenu.classList.contains("active")) {
      ...
      if (e.key === "ArrowRight") {
        ...
        items[currentFocus].click();
        dropdownMenu.classList.remove("active");
      } else if (preFocus != currentFocus && e.key === "/") {
        e.preventDefault(); // Prevents the default "/" input
...
export function initializeAditorDate(inputSection) {
  const date = new Date();
  const options = {
    month: "short",
    day: "numeric",
  ...
  inputSection.innerHTML = `
      <div class="aditor">
        ...
        <label class="headingLabel"></label>
                <div class="content">
                  <h3><ul class="inputContent" contenteditable="true">${finalFormat}</ul></h3>
                </div>
        <div class="actions">
            <button class="expand">
              <i class="fa-solid fa-caret-down"></i>
